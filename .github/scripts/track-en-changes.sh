#!/usr/bin/env bash
#
# Track changes in php/doc-en and create issues in doc-es
# for files that need translation updates.
#
# How it works:
#   - Fetches all commits on doc-en master from the last 7 days
#   - For each commit, checks if a matching issue already exists in doc-es
#   - If not, creates one listing the ES files to update
#
...
while read -r SHA; do
  ...
  if [ -z "$UPDATE_LIST" ] && [ -z "$NEW_LIST" ]; then
    echo "  -> No ES-relevant files, skipping."
    continue
  fi
  ...
  if [ -n "$UPDATE_LIST" ]; then
    BODY+=$'\n'"**Archivos ES a actualizar**"$'\n'
    BODY+="$UPDATE_LIST"
  fi

  if [ -n "$NEW_LIST" ]; then
    BODY+=$'\n'"**Nuevos archivos EN (aún no traducidos)**"$'\n'
    BODY+="$NEW_LIST"
  fi
